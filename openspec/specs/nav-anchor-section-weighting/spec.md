# Nav Anchor Section Weighting

## Purpose

Extracts nav and footer anchors from the homepage HTML to assign `navSection` and `navSource` fields to graph nodes, enabling priority-based color grouping in the graph view. Nav anchor nodes also receive visual size boosts via inbound count multipliers.
