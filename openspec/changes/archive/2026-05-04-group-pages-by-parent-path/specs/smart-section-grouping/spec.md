...
When majority inbound section detection does not yield a section (no inbound links, tie vote, or fewer than 2 inbound links total), the system SHALL fall back to the existing path-based section assignment.
...
#### Scenario: Single inbound link fallback
- **WHEN** page `/single-link` has exactly 1 inbound link from section `blog`
- **THEN** node section is `single-link` (first path segment); majority requires >50%, which for 1 link means >0.5, but we require at least 2 inbound links to trigger inference
